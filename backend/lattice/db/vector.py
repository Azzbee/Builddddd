...
class PgVectorStore:  # pragma: no cover - requires Postgres + pgvector
    """pgvector-backed store. Same interface; SQL in db/schema.sql."""

    pool: Any
    workspace_id: str = "default"
    _placeholder: list[str] = field(default_factory=list)
    ...
    async def ann_search_papers(
        ...
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                ...
                """,
                embedding, workspace_id, exclude_paper, k,
            )
            return [(r["paper_id"], r["sim"]) for r in rows]
        ...
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                ...
                ORDER BY ($4 * (1 - (embedding <=> $1))) +
                         ((1 - $4) * ts_rank(to_tsvector('english', text),
                          plainto_tsquery('english', $2))) DESC
                LIMIT $5
                """,
                query_embedding, query_text, ws, 0.6, k,
            )
            return [
                ChunkHit(
