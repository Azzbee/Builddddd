...
const STAGES = ["parsing", "extracting", "enriching", "embedding", "linking", "done"];

function StageBadges({ job }: { job: IngestJob }) {
  const reached = job.status === "succeeded" ? STAGES.length : STAGES.indexOf(job.stage) + 1;
  return (
  ...
  return (
    <div className="space-y-5">
      <header>
        ...
        <p className="text-sm text-muted">Drop a PDF or paste an arXiv id. Watch the pipeline run.</p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <label className="card flex cursor-pointer flex-col items-center justify-center gap-2 border-dashed py-10 text-center hover:border-accent">
          ...
          <span className="text-xs text-muted">parsed, extracted, enriched, embedded, linked</span>
      ...
      <section className="card">
        ...
        {jobs.length === 0 ? (
          ...
          <ul className="space-y-3">
            {jobs.map((j) => (
              <li key={j.job_id} className="rounded-md border border-border bg-panel2 p-3">
                <div className="mb-2 flex items-center justify-between text-xs">
                  <span className="font-mono text-muted">{j.paper_id ?? j.job_id.slice(0, 8)}</span>
                  <span
                    className={
                      j.status === "succeeded"
                        ? "text-good"
